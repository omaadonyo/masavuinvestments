...
new class extends Component implements HasSchemas 
{
                ...
                Step::make('Personal')
                    ...
                    ->schema([
                        Grid::make(2)->schema([
                            ...
                            Select::make('initial_investment')
                                ->options([
                                    104000 => '100,000ugx + 4,000ugx Mgt Fees',
                                ])
                                ->native(false)
    ...
    {
        $onboarding = \App\Models\Onboarding::create($this->form->getState());

        if($onboarding){
 
            //send email to admin and client
            Mail::to($onboarding->email_address)->send(new NewOnboarding($onboarding));
            Mail::to('masavuinvestmentclub@gmail.com')->send(new NewOnboarding($onboarding));
            Mail::to('info@masavuinvestments.com')->send(new NewOnboarding($onboarding));
            Mail::to('amazing.chief23@gmail.com')->send(new NewOnboarding($onboarding));

            Notification::make()
                ->title('Saved successfully')
                ->success()
                ->body('Your application has been successfully submitted, check your email for updates from the Administrator')
                ->send();
